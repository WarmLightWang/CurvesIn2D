 ...
 *
 * Designed for making quick UIs
 *
 * 
 * There are two different ways to use this:
 * 1. There is a "RunCanvas" class that is very full featured - it keeps a bunch of
 *    state, can adjust things afterwards
 * 2. There is a "runCanvas" function that creates a runCanvas and sets it up. This
 *    is easier to use. 
 * 
 * Note the convention in naming: capitalized words are used for class names
 * (e.g. RunCanvas), lower case names are used for variables and functions
 * (e.g. runCanvas).
 */
...
/**
 * the main thing is implemented as a class in case to access to everything
 */
export class RunCanvas {
  ...
  /**
   * Setup aspects of the slider - as a function in case that needs to change them
   * @param {Number} min
   * @param {Number} max
   * @param {Number} step
   */
